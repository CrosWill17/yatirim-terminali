-- =============================================================================
-- YATIRIM TERMİNALİ v3.4 — FON İÇERİĞİ (FUND HOLDINGS) ŞEMASI
--
...
-- Tüm tablolar kullanıcı yalıtımlıdır (user_id + RLS auth.uid() = user_id).
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS fund_holdings (
  ...
  source        VARCHAR(30)  NOT NULL DEFAULT 'auto', -- auto | calibration | manual | kap-pdf
  notes         TEXT,
  created_at    TIMESTAMPTZ  NOT NULL DEFAULT now(),
  updated_at    TIMESTAMPTZ  NOT NULL DEFAULT now(),
  UNIQUE (user_id, fund_code, ticker)
);
CREATE INDEX IF NOT EXISTS idx_fund_holdings_fund ON fund_holdings (user_id, fund_code);
...
CREATE INDEX IF NOT EXISTS idx_fhh_fund_ticker ON fund_holdings_history (user_id, fund_code, ticker, snapshot_at DESC);
...
CREATE TABLE IF NOT EXISTS calibration_log (
  ...
  notes               TEXT,
  created_at          TIMESTAMPTZ  NOT NULL DEFAULT now(),
  UNIQUE (user_id, fund_code, cal_date)
);
...
-- RLS: kullanıcı yalıtımı (yalnızca kendi satırları)
