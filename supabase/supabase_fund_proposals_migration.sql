...
-- Kullanıcı yalıtımlı: user_id + RLS auth.uid() = user_id
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS fund_holding_proposals (
  ...
  UNIQUE (user_id, fund_code, ticker, source_tweet_id)
);

CREATE INDEX IF NOT EXISTS idx_fhp_fund_status ON fund_holding_proposals (user_id, fund_code, status, detected_at DESC);
